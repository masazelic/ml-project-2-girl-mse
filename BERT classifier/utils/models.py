# This code is mostly taken from https://github.com/huggingface/transformers/blob/main/src/transformers/models/bert/modeling_bert.py
# Some pieces of code are added/modified by Marija Zelic, Sara Zatezalo and Elena Mrdja for the purposes of ML4Science project
import torch
...
def number_of_model_parameters(model):
    return sum([m.numel() for m in model.parameters()])

ACT2FN = {"gelu": torch.nn.functional.gelu, "relu": torch.nn.functional.relu, "swish": torch.nn.functional.silu}
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, positional_embedding):
        super().__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        if positional_embedding is not None:
            self.position_embeddings = positional_embedding(config.max_position_embeddings, config.hidden_size, config.device)
        else:
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        embeddings = (inputs_embeds + position_embeddings + token_type_embeddings).to(device)
...
class BertLayer(nn.Module):
    def __init__(self, config, attention):
        super().__init__()
        self.attention = BertAttention(config, attention)
        self.is_decoder = config.is_decoder
        if self.is_decoder:
            self.crossattention = BertAttention(config, attention)
...
class BertEncoder(nn.Module):
    def __init__(self, config, attention):
        ...
        self.layer = nn.ModuleList([BertLayer(config, attention) for _ in range(config.num_hidden_layers)])
...
class BertModel(nn.Module):
    ...
    def __init__(self, config, attention, positional_embedding):
        super().__init__()
        self.config = config
        self.embeddings = BertEmbeddings(config, positional_embedding)
        self.encoder = BertEncoder(config, attention)
...
class BertForMaskedLM(nn.Module):
    def __init__(self, config, attention, positional_embedding=None):
        super().__init__()
        self.config = config

        self.bert = BertModel(config, attention, positional_embedding)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        Standard attention layer for transformer architecture initialization. 
        For more information, refer to Attention is all you need! paper https://arxiv.org/pdf/1706.03762.pdf.
        ...
        # Using config.attention_probs_dropout_prob as the dropout probability
    ...
    def forward(
        ...
        # Get the key, query, and value from the hidden_states
        mixed_key_layer = self.key(hidden_states)
        mixed_query_layer = self.query(hidden_states)
        mixed_value_layer = self.value(hidden_states)

        # Transpose K, Q, V to get the score using self.transpose_for_scores
        query_layer = self.transpose_for_scores(mixed_query_layer)
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)

        # Get the raw attention score
        ...
        # Normalize the attention scores to probabilities and use self.dropout to do the dropout
        ...
        # Multiply each value by the score
...
class PositionalEmbedding(nn.Module):
    ...
    def __init__(self, max_position_embeddings, hidden_size, device):
        super().__init__()
        """
        Class for making positional encodings. 
        For more informations about how Positional Embedding is obtained follow the link https://kazemnejad.com/blog/transformer_architecture_positional_encoding/.
        
        max_position_embeddings: Maximum length of the input.
        hidden_size: Encoding dimension.
